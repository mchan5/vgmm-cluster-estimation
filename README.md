...
This repository is a proof of concept for the localization for coloured targets during autonomous drone missions. It uses the Variational Bayesian Gaussian Mixture Model (VBGMM) to resolve the exact centres of detected coloured targets, by combining many estimates.
...
- Filter Points by Ownership:
  - Consider the number of points each estimated centroid has. If it is less than the pre-determined mininum point sper cluster, remove that centroid.
...
This program is also structured to be implemented through the MAVLink Protocol, featuring a worker to handle all of the calculations and logic, and can be treated as an object that will do this task. To vlaidate the worker, this repository also includes a script to test the worker, using kml files collected from real-time drone flight data, or generated.
